...
function StarRate({ rate, size }) {
  ...
  return (
    <>
      {starsArray.map((fullStar) => <img key={fullStar} src={star} alt="Star" className={`mr-2 ${size === "small" ? "w-3 h-3" : ""}`} />)}
      {lastStar && <img src={getLastStar()} alt="Star" className={`mr-2 ${size === "small" ? "w-3 h-3" : ""}`} />}
    </>
  );
}
